...
//Imports da MUI
import {
    Button, Box, Avatar, TextField, Backdrop, CircularProgress,
...
const AlterarDadosPage = () => {
    ...
    React.useEffect(() => {
        ...
        document.getElementById("CEP_complemento").value = usuarioLS.CEP_complemento;
    }, []);
    ...
    const attCadastroUsuario = () => {
        ...
        if (document.getElementById("CEP_numEnd").value === " " || document.getElementById("CEP_numEnd").value === "") {
            setMsgAlerts("Endereço incompleto: Número da residência não informado!")
            setAbreNaoPode(true)
            return
        }

        Axios.put("http://localhost:1003/api/usuarios/" + locationState.usuario.email, {
            email: locationState.usuario.email,
            nome: locationState.usuario.nome,
            funcao: document.getElementById("funcao").value,
            cep: document.getElementById("CEP").value,
            numEnd: document.getElementById("CEP_numEnd").value === "" ? null : document.getElementById("CEP_numEnd").value,
            ...
            if (response.status === 201) {
                Axios.get("http://localhost:1003/api/usuarios/" + locationState.usuario.email
                ).then(responseUsuario => {
                    if (responseUsuario.status === 200) {
                        if (responseUsuario.data.status >= 1 && responseUsuario.data.status <= 4) {
                            localStorage.setItem("usuario", JSON.stringify(responseUsuario.data));
                            setUsuarioAlterado(responseUsuario.data)
                            setMensagem(locationState.usuario.nome + " atualizado com sucesso!");
                            setSair(true);
                        }
                        else
                            redirect("/?acesso=" + 0);
    ...
    const fecharDialog = () => {
        ...
        redirect("/home"//, { state: { admin: locationState.admin, usuario: usuarioAlterado } }
        );
    }
    ...
    return (
        <div className="AlterarDados">
            ...
            <ThemeProvider theme={MuiEstilosPersonalizados}>
                <div className={jaCarregou ? "container-form" : "ocultar"} style={{ backgroundColor: 'var(--nvClaroFundo)' }}
                >
                    <div className="perfil">
                        <Avatar alt={"img_" + locationState.usuario.nome}
                    ...
                    <div className="areaBtns">
                        ...
                        <Button variant="contained" color="success" name="concluir" onClick={attCadastroUsuario}>
                            Atualizar dados
                        </Button>
                    </div>
                </div>
