...
// Imports MUI e Joy
import {
    Container, Typography, Box, Button, Avatar,
    List, ListItem, ListItemText, TextField,
    CssBaseline, Select, MenuItem, Chip
} from "@mui/material";
...
const CriarTimePage = () => {
    ...
    const fecharDialog = () => {
        setSair(false);
        redirect("/times"//, { state: { admin: true, usuario: locationState.usuario } }
        );
    }
    ...
    const removeIntegrante = (integrante) => {
        const novaListaIntegrantes = listaIntegrantes.filter((item) => item !== integrante);
    ...
    const criarTimeCompleto = () => {
        ...
        if (document.getElementById('nome-time').value === '') {
            ...
            Axios.post("http://localhost:1003/api/times", {
                nome: document.getElementById("nome-time").value,
            })
                .then((respostaCriaTime) => {
                    console.log(respostaCriaTime);

                    Axios.post("http://localhost:1003/api/times/" + respostaCriaTime.data + "/usuarios", {
                    ...
                    setMensagem(document.getElementById("nome-time").value + " criado com sucesso!");
                    setSair(true);
                });
        }
    }
                            ...
                            >
                                ...
                                <Container sx={{ display: 'flex' }} className="containerDados">
                                    ...
                                    <Autocomplete
                                        ...
                                        options={listaUsuarios.filter(
                                            cbox => listaIntegrantes.every(integ => integ.email !== cbox.email)
                                        )
                                        }
                                        getOptionLabel={(option) => option.nome}
                                        sx={{ marginLeft: '20px', width: '50%' }}
                                        renderInput={(params) => <TextField {...params} label="Escolha um integrante" />}
                                        onChange={(event, usuario) => integranteChange(event, usuario)}
                                    />
                                    {/* <Select
                                        id="integranteSelecionado"
                                        variant="outlined"
                                    ...
                                    <Button onClick={exibirIntegrantes} variant="contained" sx={{ marginLeft: '20px', width: '20%' }}>
                                        Incluir
                                    </Button>
                                </Container>

                                <Container overflow={'auto'} className="containerExibeDados" sx={{ display: 'flex' }}>
                                    <List>
                                ...
                                <Container className="containerSalvar" sx={{ display: 'flex' }}>
                                    <Button color="success" variant="contained" onClick={criarTimeCompleto} sx={{ width: '20%' }} >
                                        Salvar
                                    </Button>
                                </Container>
                            </Box>
