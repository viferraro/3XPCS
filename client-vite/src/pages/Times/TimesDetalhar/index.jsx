...
// Imports MUI e Joy
import {
    Container, Typography, Box, Button, CssBaseline, Chip,
...
const DetalharTimePage = () => {
    ...
    const [locationState, setLocationState] = React.useState({ admin: '', usuario: '', time: '' });
    ...
    const retornarTime = (codTime) => {
        Axios.get("http://localhost:1003/api/time/" + codTime)
            .then((resposta) => {
    ...
    const fecharDialog = () => {
        setSair(false);
        redirect("/times"//, { state: { usuario: locationState.usuario, admin: locationState.admin } }
        )
    }
    ...
    const alterarStatusTime = () => {
        ...
        if (statusTime === true) {
            Axios.get("http://localhost:1003/api/times/" + time.codTime + "/projetosAtivos").then(
                response => {
                    if (response.status === 200) {
                        if (parseInt(response.data.qtd) > 0) {
                            setMsgAlerts("O time ainda possui " + response.data.qtd + " projetos em andamento!")
                            setAbreNaoPode(true);
                            return;
                        }
                        else {
                            Axios.put("http://localhost:1003/api/times/" + time.codTime + "/status/", {
                                ativo: !statusTime
                            })
                                .then(resposta => {
                                    setSair(true);
                                    setMensagem(time.nome + " foi " + (statusTime ? "inativado" : "reativado") + " com sucesso!");
            ...
            <div className={jaCarregou ? "mainDetalha" : "ocultar"}>
                <ThemeProvider theme={MuiEstilosPersonalizados}>
                    <Container className="principalDetalha"
                        ...
                        <Box height={'100%'} overflow={'auto'} className="boxDetalha" sx={{ display: 'flex' }}>
                            ...
                            <Container className="detalhaTime" disableGutters sx={{ display: 'flex', flexDirection: 'row' }}>
                                <Container disableGutters className="detalhaIntegrantetime">
                                    <Typography variant="h3" component="h2" sx={{ color: 'subTituloDetalha', fontWeight: '500' }}>
                                        Integrantes
                                    </Typography>
