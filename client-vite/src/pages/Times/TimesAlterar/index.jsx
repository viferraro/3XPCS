...
// Imports MUI e Joy
import {
    Container, Typography, Box, Button, List,
    ListItem, ListItemText, TextField, CssBaseline,
    Select, MenuItem, Snackbar, Alert, AlertTitle, Chip,
    Backdrop, CircularProgress, Autocomplete
} from "@mui/material";
...
const AlterarTimePage = () => {
    ...
    const [locationState, setLocationState] = React.useState({ admin: '', usuario: '', timeAlterar: '' });
    ...
    React.useEffect(() => {
        ...
        setLocationState({ admin: adminLS, usuario: usuarioLS, timeAlterar: timeAlterarLS })
    ...
    const [listaUsuariosComboBox, setListaUsuariosComboBox] = React.useState([{ nome: '', email: '' }]);

    // ! Função que busca e enche o combobox de Usuarios
    const buscarTodosUsuarios = () => {
        Axios.get("http://localhost:1003/api/usuarios-ativos").then(
            response => {
    ...
    const inserirNovoIntegrante = () => {
        Axios.post("http://localhost:1003/api/times/" + time.codTime + "/usuarios", {
    ...
    const removeIntegrante = (integrante) => {
        ...
        Axios.put("http://localhost:1003/api/times/" + time.codTime + "/usuarioDelete", {
            codTime: time.codTime,
    ...
    const finalizarTimes = () => {
        Axios.put("http://localhost:1003/api/times/" + time.codTime + "/nome", {
            nome: document.getElementById("nome-time").value,
        }).then((respostaAlteraNomeTime) => {
            console.log(respostaAlteraNomeTime)
            //TERIA QUE VERIFICAR SE TA ADICIONANDO MESMO...

            setMensagem(document.getElementById("nome-time").value + " atualizado com sucesso!");
            setSair(true);
            retornarTime(time.codTime);
        });
    }
                            ...
                            <Box className="boxNovoTime"
                                ...
                                <Container className="containerDados" sx={{ display: 'flex' }} >
                                    ...
                                    <Autocomplete
                                        ...
                                        options={listaUsuariosComboBox.filter(
                                            cbox => listaIntegrantes.every(integ => integ.email !== cbox.email)
                                        )}
                                        getOptionLabel={(option) => option.nome}
                                        sx={{ marginLeft: '20px', width: '50%' }}
                                        renderInput={(params) => <TextField {...params} label="Integrantes" />}
                                    ...
                                    <Button
                                        ...
                                        variant="contained" sx={{ marginLeft: '20px', width: '20%' }}>
                                        Incluir
                                    </Button>
                                </Container>

                                <Container className="containerExibeDados" sx={{
                                    display: 'flex',
