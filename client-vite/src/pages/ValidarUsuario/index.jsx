...
const ValidarUsuario = () => {
    ...
    const vrfUsuario = () => {
        ...
        Axios.put("http://localhost:1003/api/usuarios/" + params.get('email').toString() + "/valida", {
            urlImagem: params.get('foto').toString(),
            nome: params.get('nome').toString(),
        }, { validateStatus: false }).then(response => {
            ...
            Axios.get("http://localhost:1003/api/usuarios/" + params.get('email').toString()
            ).then(response => { //TESTE SETANDO LOCALSTORAGE
                switch (response.data.status) {
                    case 1:
                        delay(3000).then(() => {
                            localStorage.setItem("usuario", JSON.stringify(response.data))
                            localStorage.setItem("admin", false)
                            redirect("/cadastro"//, { state: { usuario: response.data, admin: false } }
                            )
                        }
                        );
                        break;
                    case 2:
                        delay(3000).then(() => {
                            ...
                            redirect("/cadastro"//, { state: { usuario: response.data, admin: true } }
                            )
                        })
                        break;
                    case 3:
                        ...
                        delay(3000).then(() => {
                            localStorage.setItem("usuario", JSON.stringify(response.data))
                            localStorage.setItem("admin", false)
                            redirect("/home"//, { state: { usuario: response.data, admin: true } }
                            )
                        })
                        break;
                    case 4:
