...
import {
    ...
    CssBaseline, Grid,
    Snackbar, Alert, AlertTitle, Chip,
    Tooltip, Backdrop, CircularProgress, Autocomplete
} from "@mui/material";
...
const ProjetosAlterar = () => {
    ...
    const buscarTodosTimes = (timeAtual) => {
        Axios.get("http://localhost:1003/api/timesAtivos").then(
            response => {
    ...
    const alteraProjeto = () => {
        ...
        if (locationState.projeto.timeResponsavel !== codTimeSelecionado) {
            Axios.get("http://localhost:1003/api/projetos/" + locationState.projeto.codProjeto + "/qtdtarefas").then(
                (resposta) => {
                    if (resposta.data.qtdTarefas > 0) {
                        setMsgAlerts("Não é possível alterar o time, pois ainda há tarefas ativas associadas a ele!")
                        setAbreNaoPode(true);
                        return;
                    }

                    if (document.getElementById('nome-projeto').value.trim() === '' || eNomeValido === false) {
                        setAbreNaoPode(true)
                    ...
                    if (listaIteracoes.length <= 1) {
                        setAbreNaoPode(true)
                        setMsgAlerts("É necessário criar ao menos 1 iteração (sem contar a ITERAÇÃO PADRÃO)!")
                        return;
                    }

                    else {
                        Axios.put("http://localhost:1003/api/projetos/" + locationState.projeto.codProjeto, {
                            ...
                            .then((respostaValida) => {
                                ...
                                if (respostaValida.status === 200) {
                                    setSair(true);
                                    setMensagem(document.getElementById("nome-projeto").value + " foi atualizado!");
                                    setSaiDaPagina(true)
                                }
                                else {
                                    setMsgAlerts("Ocorreu algum erro ao alterar o projeto: \n" + respostaValida.data.message)
                                    setAbreNaoPode(true);
                                }
                            });
                    }
    ...
    const criarIteracao = (nome, descricao, dtInicio, dtFim) => {
        Axios.post("http://localhost:1003/api/projetos/" + locationState.projeto.codProjeto + "/iteracao",
            {
                nome: nome,
                descricao: descricao,
                dtInicio: dtInicio,
    ...
    const abreIteraAlteracao = (iteracao) => {
        ...
        if (iteracao.descricao !== "ITERAÇÃO GERAL DO PROJETO " + locationState.projeto.nome.toUpperCase()) {
            Axios.get("http://localhost:1003/api/projetos/" + iteracao.codIteracao + "/iteracao").then(
                response => {
                    console.log(response.data)
    ...
    const alterarIteracao = (codIteracao, nome, descricao, dtInicio, dtFim) => {
        Axios.put("http://localhost:1003/api/projetos/" + codIteracao + "/iteracao",
            {
                nome: nome,
                descricao: descricao,
                dtInicio: dtInicio,
    ...
    const removeIteracao = (iteracao) => {
        ...
        if (iteracao.hasOwnProperty('codIteracao')) {
            if (iteracao.descricao !== "ITERAÇÃO GERAL DO PROJETO " + locationState.projeto.nome.toUpperCase()) {
                ...
                Axios.get("http://localhost:1003/api/tarefas-itera/" + iteracao.codIteracao).then(
                    response => {
                        if (response.data.qtdTarefas === 0) {
                            //Remove a iteração da tabela...
                            Axios.delete("http://localhost:1003/api/projetos/" + iteracao.codIteracao + "/iteracao").then(
                                resposta => {
                                    if (resposta.status === 201) {
                                        ...
                                        //Ajustando lista
                                        let listaNova = listaIteracoes.filter(
                                            function (item) {
                            ...
                            setMsgAlerts("A iteração ainda possui " + response.data.qtdTarefas + " tarefas associadas.")
                            setAbreNaoPode(true);
                        }
                    });
            }
            ...
            const novaListaIteracoes = listaIteracoes.filter((item) => item !== iteracao);
    ...
    const validarNome = () => {
        ...
        else if (document.getElementById("nome-projeto").value.trim() === locationState.projeto.nome) {
            setENomeValido(true);
            setLabelNome('Válido!');
        }
        else {
            Axios.put("http://localhost:1003/api/projetos/" + document.getElementById("nome-projeto").value + "/validaNome")
                .then((respostaValida) => {
                    console.log(respostaValida);
    ...
    const getDescricao = (codProjeto) => {
        Axios.get("http://localhost:1003/api/projetos/" + codProjeto + "/descricao").then(
            response => {
                document.getElementById('txt-descricao').value = (response.data.descricao);
            });
    }
        ...
        <>
            ...
            <div className="mainCria">
                ...
                >
                    <ThemeProvider theme={MuiEstilosPersonalizados}>
                        ...
                        <Container sx={{ height: '100%' }} maxWidth="md">
                            ...
                            >
                                ...
                                <Container sx={{ height: 'fit-content', display: 'flex', flexDirection: 'column', alignItems: 'center' }} maxWidth="md">
                                            ...
                                            if (iteracao.hasOwnProperty('nome')) {
                                                return (
                                                    <Container gutterBottom sx={{ width: '260px' }}>
                                                        <Tooltip title={iteracao.nome} placement="top" arrow>
                                                            <Chip
                                ...
                                <Container className="containerSalvar" sx={{ display: 'flex' }}>
                                    <Button color="success" variant="contained" onClick={alteraProjeto}
                                        sx={{ width: '20%' }} >
                                        Salvar
                                    </Button>
                                </Container>
                    ...
                    <AlteraIteracao saiItera={saiIteraAlteracao} sairItera={sairIteraAlteracao} alterarIteracao={alterarIteracao} iteracao={iteracaoAlterada} />
                    <CriarIteracao saiItera={saiItera} sairItera={sairItera} criarIteracao={criarIteracao} />
                </Container>
                <PopupOKPersonalizado sair={sair} fecharDialog={fecharDialog} mensagem={mensagem} />
                ...
                <Snackbar
                    ...
                    <Alert onClose={fechaAlertAdd} severity="success" sx={{ width: '100%' }}>
