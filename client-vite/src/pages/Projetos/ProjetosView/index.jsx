...
// Imports MUI e Joy
import {
    Grow, Container, Typography, Switch, Card, Button,
    Link, CardContent, CardActions, Badge, Box,
    Backdrop, CircularProgress, Grid, Tooltip
} from "@mui/material";
import { BadgeUnstyled } from "@mui/base";
import { ThemeProvider } from "@mui/material/styles";
import { FaPlusCircle, FaPencilAlt, FaCircle, FaHandPointLeft } from 'react-icons/fa';
...
const ProjetosView = () => {
    ...
    const buscarTodosProjetos = () => {
        Axios.get("http://localhost:1003/api/usuarios/" + "-1" + "/projetos").then(
            response => {
    ...
    const buscarProjetos = (email) => {
        Axios.get("http://localhost:1003/api/usuarios/" + email + "/projetos").then(
            response => {
                console.log(response.data);// TESTE - 
                setListaProjetos(response.data);
                setJaCarregou(true)
    ...
    const alterarDadosProjeto = (projeto => {
        ...
        redirect("/projetos/alterar"//, { state: { usuario: locationState.usuario, admin: locationState.admin, projeto: projeto } }
        )
    });
    ...
    const detalharProjeto = (projeto => {
        ...
        redirect("/projetos/detalhar"//, { state: { usuario: locationState.usuario, admin: locationState.admin, projeto: projeto } }
        )
    });
    ...
    const [textoSwitch, setTextoSwitch] = React.useState("Mudar para meus projetos");
            ...
            <div className={jaCarregou ? "principalTimes" : "ocultar"}>
                <ThemeProvider theme={MuiEstilosPersonalizados}>
                    <Container disableGutters sx={{ paddingTop: '0' }} maxWidth="md">
                        <Container className="containerVolta" disableGutters sx={{
                            display: 'flex',
                            alignItems: 'center',
                            justifyContent: 'flex-start',
                        }}>
                            <Button className={locationState.admin === "true" ? "" : "ocultar"}
                                color="success"
                                variant="contained"
                                size="large"
                                startIcon={<FaPlusCircle />}
                                onClick={() => {
                                    redirect("/projetos/criar"//, { state: { admin: true, usuario: locationState.usuario } }
                                    )
                                }}
                        ...
                        <Grid overflow={'auto'} maxHeight={'70vh'} className="gridTimes" container spacing={4}
                            sx={{ margin: ' 1.5em 0 1.5em 0 ' }}>

                            <Container disableGutters sx={{
                                paddingTop: '0', display: 'flex', flexDirection: 'row', alignItems: 'center'
                            }}
                                    ...
                                    return (
                                        ...
                                        >
                                            <Box sx={{
                                                ...
                                                <Card sx={{
                                                    ...
                                                    <CardContent>
                                                        ...
                                                        >
                                                            {"Coordenação: "}
                                                            <Typography
                                                                sx={{ mb: 1, fontWeight: '500', display: 'inline-block' }}
                                                                color="texto.descricao"
                                                            >
                                                                {projeto.nomeTime}
                                                            </Typography>
                                                    ...
                                                    <CardActions>
                                                        {
                                                            <Button
                                                                className={locationState.admin === "true" &&
                                                                    projeto.ativo.toString() === "true" && projeto.dtConclusao === null
                                                                    ? "" : "ocultar"}
                                                                onClick={() => { alterarDadosProjeto(projeto) }}
                                                                color="warning"
                                                                variant="contained"
                                                                size="small"
                                                ...
                                                <Tooltip title={projeto.ativo ? (projeto.dtConclusao === null ? "Em andamento" : "Concluído") : "Inativo"} 
                                                    placement="top"
                                                    arrow
                                                >
                                                    <Badge badgeContent={
                                                        <div >
                                                            <FaCircle color={projeto.ativo ? (projeto.dtConclusao === null ? "#f3cc30" : corStatus[projeto.ativo.toString()]) : corStatus[projeto.ativo.toString()]} fontSize="1.7em" />
                                                        </div>
                                                    }
                                                    >
                                                    </Badge>
