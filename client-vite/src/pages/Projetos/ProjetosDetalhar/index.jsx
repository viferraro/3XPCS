...
// Imports MUI e Joy
import {
    Box, Container, Typography, Slide, Grid,
    Button, Link, List, ListItem, ListItemText, CssBaseline,
    Backdrop, CircularProgress,
    Snackbar, Alert, AlertTitle
} from "@mui/material";
import { ThemeProvider } from "@mui/material/styles";
import { FaHandPointLeft, FaUserShield, FaUserTimes, FaBookOpen, FaCheckCircle } from 'react-icons/fa';
...
const ProjetosDetalhar = () => {
    ...
    const [locationState, setLocationState] = React.useState({ admin: '', usuario: '', projeto: '' });
    ...
    React.useEffect(() => {
        ...
        if (localStorage.hasOwnProperty("admin") === false || localStorage.hasOwnProperty("usuario") === false) {
            redirect("/?acesso=" + 0);
            return;
        }
        ...
        setLocationState({ admin: adminLS, usuario: usuarioLS, projeto: projetoLS })
    ...
    const fecharDialog = () => {
        setSair(false);
        if (mensagem.toString() === (locationState.projeto.nome + " foi concluído com sucesso!").toString())
            redirect(-1);
    }
    ...
    const alterarStatusProjeto = () => {
        console.log("🚀 ~ file: index.jsx:85 ~ React.useEffect ~ projeto", locationState.projeto)

        if (statusProjeto === true) {
            Axios.get("http://localhost:1003/api/projetos/" + locationState.projeto.codProjeto + "/tarefasAtivas").then(
                response => {
                    if (response.status === 200) {
                        if (parseInt(response.data.qtd) > 0) {
                            setMsgAlerts("O projeto ainda possui " + response.data.qtd + " tarefas em andamento!")
                            setAbreNaoPode(true);
                            return;
                        }
                        else {
                            Axios.put("http://localhost:1003/api/projetos/" + locationState.projeto.codProjeto + "/status", {
                                ativo: !statusProjeto
                            })
                                .then(resposta => {
                                    setSair(true);
                                    setMensagem(locationState.projeto.nome + " foi " + (statusProjeto ? "inativado" : "reativado") + " com sucesso!");
    ...
    const visualizarTarefas = () => {
        ...
        redirect('/projetos/tarefas'//, { state: { usuario: locationState.usuario, admin: locationState.admin, projeto: locationState.projeto } }
        )
    }
    ...
    const getIteracoes = (codProjeto) => {
        Axios.get("http://localhost:1003/api/projetos/" + codProjeto + "/iteracoes").then(
            response => {
    ...
    const detalharTime = (codTime) => {
        if (codTime !== null) {
            Axios.get("http://localhost:1003/api/time/" + codTime).then(
                response => {
                    if (response.status === 200) {
                        //Setando time no localStorage
                        localStorage.setItem("time", JSON.stringify(response.data))

                        redirect("/times/detalhar"//, { state: { usuario: locationState.usuario, admin: locationState.admin, time: response.data } }
                        )
                    }
                });
        }
            ...
            response => {
                if (response.status === 200) {
                    if (parseInt(response.data.qtd) > 0) {
                        ...
                        Axios.delete("http://localhost:1003/api/projetos/" + locationState.projeto.codProjeto + "/conclui")
                            .then(resposta => {
                                setSair(true);
                                setMensagem(locationState.projeto.nome + " foi concluído com sucesso!");
        ...
        <>
            ...
            <div className={jaCarregou ? "mainDetalha" : "ocultar"}>
                ...
                <ThemeProvider theme={MuiEstilosPersonalizados}>
                    <Container className="principalDetalha"
                        maxWidth="md"
                        sx={{ paddingBottom: '24px', display: "flex", backgroundColor: 'fundoCard' }}>

                        <Box height={'100%'} className="boxDetalha" sx={{ display: 'flex' }}>
                            <Container disableGutters className="tituloDetalha">
                                <Typography variant="h2" component="h1" sx={{ color: 'tituloDetalha', fontWeight: '600' }}>
                                    {locationState.projeto.nome}
                                </Typography>
                            </Container>

                            <Container className="detalhaTime" disableGutters sx={{ display: 'flex' }}>
                                <Container disableGutters className="detalhaIntegrantetime">
                                    <Typography variant="body" component="h2" sx={{ color: 'subTituloDetalha', fontWeight: '500' }}>
                                        {locationState.projeto.descricao}
                                    </Typography>
                                </Container>

                                <Container disableGutters sx={{ display: 'flex', height: '100px', gap: '1.5em' }}>
                                    <Container disableGutters className="detalhaProjetotime" sx={{ display: 'flex', flexDirection: 'column' }}>
                                        <Typography variant="body" component="h2" sx={{ color: 'subTituloDetalha', fontWeight: '500' }}>
                                            {"Criado em: "}
                                        </Typography>
                                        <Typography variant="body" component="h2" sx={{ color: 'subTituloDetalha', }}>
                                    ...
                                    <Container disableGutters className="detalhaProjetotime" sx={{ display: 'flex', flexDirection: 'column' }}>
                                        <Typography variant="body" component="h2" sx={{ color: 'subTituloDetalha', fontWeight: '500' }}>
                                            {locationState.projeto.dtConclusao === null ? "Prazo:" : "Concluído em:"}
                                        </Typography>

                                        <Typography variant="body" component="h2" sx={{ color: 'subTituloDetalha', }}>
                                            {locationState.projeto.dtConclusao === null ? locationState.projeto.Prazo : locationState.projeto.dtConclusao}
                                        </Typography>
                                    </Container>
                                </Container>
                                ...
                                <Container
                                    ...
                                    sx={{ display: 'flex', height: '300px', width: '100%', padding: '0%' }}
                                >
                                    <Typography variant="body" component="h2" sx={{ color: 'subTituloDetalha', fontWeight: '500' }}>
                                        Iterações:
                                    </Typography>
                                    <Grid overflow={'overlay'} sx={{ width: '100%', overflowX: 'hidden' }}>
                                        <List>
                                            {listaIteracoes.map((iteracao, index) => {
                                                return (
                                                    <Slide direction="left" in={true} timeout={(index + 1) * 250} mountOnEnter unmountOnExit>
                                                        <ListItem
                                                        ...
                                                        >
                                                            <ListItemText
                                                                primary={iteracao.nome + ": de " + formatData(iteracao.dtInicio) + " a " + formatData(iteracao.dtFim)}
                                                                sx={{ color: 'subTituloDetalha', textDecoration: 'underline' }}
                                                            />
                                                        </ListItem>
                                                    </Slide>
                                                );
                        ...
                        <Container className="divBotaoFim" disableGutters sx={{ display: 'flex' }}>
                            <Button variant="contained" color="info" startIcon={<FaHandPointLeft />} onClick={voltar}>
                                Voltar
                            </Button>

                            <Button variant="contained" color="secondary" startIcon={<FaBookOpen />} onClick={visualizarTarefas}>
                                Visualizar Tarefas
                            </Button>

                            <Button
                                className={locationState.admin === "true" ?
                                    (locationState.projeto.dtConclusao === null && locationState.projeto.ativo === true ? "botaoReativar" : "ocultar")
                                    : "ocultar"}
                                variant="contained"
                                color="success"
                            ...
                            <Button
                                className={locationState.admin === "true" ? "botaoReativar" : "ocultar"}
                                variant="contained"
