...
//Imports da MUI
import {
    Button, Box, Avatar, TextField, FormControlLabel, Checkbox, Container,
    Snackbar, Alert, AlertTitle, Typography, CssBaseline
} from '@mui/material';
import { ThemeProvider } from '@mui/material/styles';


//Import de icones
import { FaUserShield, FaUserEdit, FaUserTimes, FaHandPointLeft } from 'react-icons/fa';
...
const UsuarioAlterar = () => {
    ...
    const [locationState, setLocationState] = React.useState({ admin: '', usuario: '', usuarioAlterar: '' });
    ...
    React.useEffect(() => {
        ...
        setLocationState({ admin: adminLS, usuario: usuarioLS, usuarioAlterar: usuarioAlterarLS })
    ...
    const alterarDadosUsuarioSelecionado = () => {
        ...
        if (document.getElementById("email").value.trim().toString() === locationState.usuarioAlterar.email.toString()) {
            Axios.put("http://localhost:1003/api/usuarios-adm/" + locationState.usuarioAlterar.email, {
                ...
                setSair(true);
                setMensagem((locationState.usuarioAlterar.nome ?? 'Usuário ') + " alterado com sucesso!");
            }
            );
            return;
        }
        //! Validando se usuário já existe
        Axios.get("http://localhost:1003/api/usuarios/" + document.getElementById("email").value.trim())
            .then(
                response => {
    ...
    const ativarUsuario = () => {
        Axios.put("http://localhost:1003/api/usuarios/" + locationState.usuarioAlterar.email + "/status", {
            status: eAdmin ? 2 : 1, //Se for selecionado no checkbox admin, defina o status como 2, senão, defina como 1 
        }).then(response => {
            setSair(true);
            setMensagem((locationState.usuarioAlterar.nome ?? 'Usuário ') + " reativado com sucesso!");
    ...
    const inativarUsuario = () => {
        Axios.put("http://localhost:1003/api/usuarios/" + locationState.usuarioAlterar.email + "/status", {
            ...
            setMensagem(locationState.usuarioAlterar.nome + " inativado com sucesso!");
        }
        );
    };
    ...
    return (
        <div className="Usuarios-ADM">
            ...
            <CssBaseline />
            <Container disableGutters className={"principalAlteraUsuario"} sx={{ display: 'flex', width: '30%' }}>
                <ThemeProvider theme={MuiEstilosPersonalizados}>
                    ...
                    <Container disableGutters className={"principalAlteraUsuario"} sx={{ display: 'flex', paddingTop: '0' }}>
                        <Box className="boxNovoTime"
                            sx={{
                                backgroundColor: 'var(--nvClaroFundo)', display: 'flex', flexDirection: 'column',
                                alignItems: 'stretch', borderRadius: '15px', padding: '2vh 0 5vh 0', width: '100%',
                                marginTop: '0'
                            }}
                            noValidate
                            autoComplete="off"
                        >
                            <div className="perfil">
                                <Avatar alt={"img_" + locationState.usuarioAlterar.nome} src={locationState.usuarioAlterar.urlImagem} referrerPolicy="no-referrer" />
                                <p>{locationState.usuarioAlterar.nome}</p>
                            </div>
                            <Container maxWidth="sm" sx={{ gap: '2vh', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                                <TextField
                                    className=""
                                    id="email"
                                    label="Email"
                                ...
                                <FormControlLabel
                                    label="Administrador?"
                                    control={
                                        <Checkbox label={"Administrador?"} checked={eAdmin} color="info" id="eAdmin" onChange={attStatusAdmin} />
                                    }
                                />
                                ...
                                >
                                    {
                                        usuarioInativo ?
                                            <Button className="mostrarBtn" variant="contained" color="warning" startIcon={<FaUserShield />}
                                                onClick={ativarUsuario}
                                            >
                                                REATIVAR
                                            </Button>
                                            : <Button className="mostrarBtn" variant="contained" color="error" startIcon={<FaUserTimes />}
                                    ...
                                    <Button variant="contained" color="info" startIcon={<FaUserEdit />}
