...
//Import de icones
import { FaUserPlus, FaTimesCircle, FaCheckCircle, FaUserEdit, FaHandPointLeft, FaUsers } from 'react-icons/fa';
...
const UsuarioAdm = () => {
    ...
    const getUsuarios = () => {
        Axios.get("http://localhost:1003/api/usuarios").then(
            response => {
                ...
                response.data.forEach(elemento => {
                    ...
                    listaFiltrados.push({ id: contador, email: email, nome: nome, admin: admin, ativo: ativo });
                    contador = contador + 1;
                });
    ...
    const columns = [
        { field: 'nome', headerName: 'Nome', headerClassName: 'headerTBL', flex: 3 },
        { field: 'email', headerName: 'E-mail', headerClassName: 'headerTBL', flex: 2 },
        {
            field: 'admin', headerName: 'É administrador?', headerClassName: 'headerTBL', flex: 1,
            renderCell: (cellValues) => {
                if (cellValues.row.admin === true)
                    return <div style={{ color: 'green' }}><FaCheckCircle color="green" /> Sim </div>;
                else
                    return <div style={{ color: 'red' }}><FaTimesCircle color="red" /> Não </div>;
            }
        },
        {
            field: 'ativo', headerName: 'Ativo?', headerClassName: 'headerTBL', flex: 1,
            renderCell: (cellValues) => {

                if (cellValues.row.ativo === true)
                    return <div style={{ color: 'green' }} value="Sim"><FaCheckCircle color="green" /> Sim </div>;
                else
                    return <div style={{ color: 'red' }} value="Não"><FaTimesCircle color="red" /> Não </div>;
            }
        },
    ];
    ...
    const [temUsuarioSelecionado, setTemUsuarioSelecionado] = React.useState(false);
    ...
    const alterarDadosUsuarioSelecionado = () => {
        ...
        redirect("/usuarios-adm/alterar"//, { state: { usuario: locationState.usuario, admin: locationState.admin, usuarioAlterar: usuarioSelecionado } }
        )
    }

    // ! Função para levar o usuário logado a página de criar novo usuário
    const criarNovoUsuario = () => {
        redirect("/usuarios-adm/criar"//, { state: { usuario: locationState.usuario, admin: locationState.admin } }
        )
    }
        ...
        <div className="tarefas-adm">
            ...
            <div className={jaCarregou ? "principal-tarefas-adm" : "ocultar"}>
                <ThemeProvider theme={MuiEstilosPersonalizados}>
                    <Container disableGutters className="divBotaoAdd" sx={{ paddingBottom: '1.5em', margin: '0', display: 'flex', justifyContent: 'space-between' }}>
                        <Button variant="contained" color="success" startIcon={<FaUserPlus />} onClick={criarNovoUsuario} sx={{ display: 'flex' }}>
                            ADICIONAR USUÁRIO
                        </Button>
                    </Container>

                    <Container disableGutters className="gridTarefas" sx={{ margin: '0' }}>
                        <DataGrid className={"tbl"}
                            ...
                            onSelectionModelChange={(ids) => {
                                ...
                                setUsuarioSelecionado(listaUsuarios[parseInt(idSelecionado) - 1])
                    ...
                    <Container disableGutters className="divBotaoAdd" sx={{
                    ...
                    >
                        ...
                        <Button className="botaoAlterar" variant="contained" color="success"
                            startIcon={<FaUsers />} sx={{ display: 'flex' }}
                            onClick={usuariosSeguidos}
                        >
                            USUÁRIOS QUE SIGO
                        </Button>

                        <Button className={temUsuarioSelecionado ? "botaoAlterar" : "ocultar"} variant="contained" color="warning" startIcon={<FaUserEdit />} sx={{ display: 'flex' }}
