...
//Imports da MUI
import {
    Button, Box, Avatar, Backdrop, CircularProgress,
    Typography, Divider, Chip, Rating, TextField, Grow, Card, CardContent, CardHeader,
...
const UsuarioPerfil = () => {
    ...
    const [locationState, setLocationState] = React.useState({ admin: '', usuario: '', usuarioPerfil: '' });
    ...
    React.useEffect(() => {
        ...
        if (localStorage.hasOwnProperty("admin") === false
        ...
        setLocationState({ admin: adminLS, usuario: usuarioLS, usuarioPerfil: usuarioPerfilLS })
    ...
    const buscaUsuario = (email) => {
        Axios.get("http://localhost:1003/api/usuarios/" + email)
            .then(
                response => {
    ...
    const vrfUsuarioESeguido = (emailUsuarioLogado, usuarioSeguido) => {
        ...
        Axios.get("http://localhost:1003/api/usuarios/" + emailUsuarioLogado + "/seguindo/" + usuarioSeguido)
            .then(
                response => {
                    setVrfSeguido(true)
    ...
    const deixardeSeguirUsuario = () => {
        Axios.put("http://localhost:1003/api/usuarios/" + locationState.usuario.email + "/usuario/parar-seguir", {
    ...
    const seguirUsuario = () => {
        Axios.put("http://localhost:1003/api/usuarios/" + locationState.usuario.email + "/usuario/seguir", {
            email: usuarioPerfil.email,
        }).then(
            response => {
                if (response.status === 200) {
    ...
    const buscarAvs = (email) => {
        Axios.get("http://localhost:1003/api/usuarios/" + email + "/avaliacoes-recebidas").catch(error => {
    ...
    const avaliarUsuario = () => {
        if (avaliacao == 0) {
            setMsgAlerts("Avaliação inválida, por favor, selecione ao menos uma estrela!")
            setAbreNaoPode(true);
            return;
        }

        Axios.post("http://localhost:1003/api/usuarios/" + locationState.usuario.email + "/avaliacoes", {
            email: usuarioPerfil.email,
            avaliacao: avaliacao,
    ...
    return (
        <div className="Usuarios-ADM">
            ...
            <ThemeProvider theme={MuiEstilosPersonalizados}>
                <Container className={vrfSeguido ? "principalAlteraUsuario" : "ocultar"} sx={{ display: 'flex' }}>
                    <Container disableGutters className="containerPerfilUsuario" sx={{ display: 'flex' }}>
                        <Container className={"container-formulario2"}
                            sx={{
                        ...
                        >
                            ...
                            <div className="perfil">
                                <Avatar alt={"img_" + usuarioPerfil.hasOwnProperty('nome') ? usuarioPerfil.nome : ""}
                                    sx={{ width: 100, height: 100 }}

                                    src={usuarioPerfil.hasOwnProperty('urlImagem') ? usuarioPerfil.urlImagem : ""}
                                    referrerPolicy="no-referrer" />
                                <h4><b>{usuarioPerfil.hasOwnProperty('nome') ? usuarioPerfil.nome : ""}</b></h4>
                                <p>{usuarioPerfil.hasOwnProperty('email') ? usuarioPerfil.email : ""}</p>
                            </div>
                            <div>
                            ...
                            >
                                <b>Função:</b> {usuarioPerfil.hasOwnProperty('funcao') ? usuarioPerfil.funcao : ""}
                            </Typography>
                            ...
                            <Box
                                ...
                                >
                                    <a
                                        ...
                                        className={usuarioPerfil.hasOwnProperty('linkedin') ? (usuarioPerfil.linkedin === '' ? 'ocultar' : '') : 'ocultar'}
                                        href={usuarioPerfil.hasOwnProperty('linkedin') ? usuarioPerfil.linkedin : ''} target="_blank"
                                ...
                                >
                                    <a
                                        className={usuarioPerfil.hasOwnProperty('github') ? (usuarioPerfil.github === '' ? 'ocultar' : '') : 'ocultar'}
                                        href={usuarioPerfil.hasOwnProperty('github') ? usuarioPerfil.github : ''} target="_blank"
                                        style={{ backgroundColor: 'var(--nvClaroFundo)' }}
                                    //className={usuarioPerfil.hasOwnProperty('linkedin') ? (usuarioPerfil.linkedin === '' ? "" : "link-underline-transicao") : ""} // Se a tarefa for do usuario, aparece normal
                                    >
                            ...
                            <div className="areaBtns" style={{ margin: '1em', }}>
                                ...
                                {!(usuarioProprioPerfil) ? <div>
                                    <Button variant="contained" color="success" name="dxseguir"
                                    ...
                                    <Button variant="contained" color="success" name="seguir"
                                        className={usuarioESeguido ? "ocultar" : ""}
                        ...
                        <Container className={'container-formulario2'} disableGutters maxWidth='sm'
                            sx={{
                                display: usuarioProprioPerfil ? 'none' : 'flex', flexDirection: 'column', paddingBottom: '1em',
                                justifyContent: 'space-between', backgroundColor: "var(--nvClaroFundo)"
                            }}
                        >
                            ...
                            <Container disableGutters sx={{ height: '40%', marginBottom: '2.5em' }}>
                                <Divider sx={{ margin: '1em 0 1em 0' }}>
                                ...
                                <Box sx={{ display: 'flex', paddingLeft: '1em' }}>
                                    ...
                                    <Rating value={avaliacao} name="avaliacaoEstrelas"
                                ...
                                <Container disableGutters widht='md'
                                    sx={{ display: 'flex', paddingLeft: '1em', paddingRight: '1em' }}
                                >
                                    <Typography
                                        gutterBottom
                                        vaiant="h6"
                                    ...
                                    <TextField multiline fullWidth minRows={3} maxRows={3}
                                        id="avaliacao-comm">
                                ...
                                <Container disableGutters className="divBotaoFim" sx={{ display: 'flex', justifyContent: 'flex-end', paddingRight: '1em' }}>
                                    <Button variant="contained" color="info" startIcon={<FaCommentDots />} onClick={avaliarUsuario}>
                                        Avaliar
                                    </Button>
                                </Container>
                        ...
                        <Container className={'container-formulario2'} disableGutters maxWidth='sm'
                            sx={{
                                display: avs[0].hasOwnProperty("nome") ? 'flex' : 'none', flexDirection: 'column', paddingBottom: '1em',
                                ...
                                return (
                                    ...
                                    >
                                        <Card sx={{
                                            ...
                                            className="cardTimes" variant="outline">

                                            <CardHeader
                                                avatar={
                                                    <Avatar
                                                        src={avaliacao.hasOwnProperty('urlImagem') ? avaliacao.urlImagem : "A"}
                                                        referrerPolicy="no-referrer"
                                                    />
                                                }
                                                title={avaliacao.hasOwnProperty("nome") ? avaliacao.nome : ""}
                                                subheader={<Rating value={parseInt(avaliacao.avaliacao)}
                                                    key={"RAT_MIN_" + index}
                                                    readOnly
                                                />}
                                            />
                                            <CardContent>
                                                <Typography
                                                    sx={{ mb: 1, fontWeight: '600', display: 'inline-block' }}
                    ...
                    <Container disableGutters className="divBotaoFim" sx={{ display: 'flex', justifyContent: 'flex-start', gridArea: 'main' }}>
            ...
            <PopupOKPersonalizado sair={sair} fecharDialog={fecharDialog} mensagem={mensagem} />
        </div >
    )
}
