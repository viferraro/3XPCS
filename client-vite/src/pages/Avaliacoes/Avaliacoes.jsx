...
//Imports da MUI
import {
    Button, Backdrop, CircularProgress, Grow, Container, Grid, CssBaseline,
    Tabs, Tab, Card, CardContent, Rating, Typography, CardHeader, Avatar,
...
const UsuarioCriar = () => {
    ...
    const buscarMinhasAvs = (email) => {
        Axios.get("http://localhost:1003/api/usuarios/" + email + "/avaliacoes-recebidas").then(
            response => {
    ...
    const buscarAvsFeitas = (email) => {
        Axios.get("http://localhost:1003/api/usuarios/" + email + "/avaliacoes-feitas").then(
            response => {
                    ...
                    <div className={jaCarregou ? "divMeio" : "ocultar"}>
                        ...
                        <Container disableGutters sx={{ paddingTop: '0' }} maxWidth="md">
                            <Container className="containerVolta" disableGutters sx={{ display: 'flex', justifyContent: 'space-between' }}>
                                <Tabs value={tipoAv} onChange={mudaVisualizacao}
                                    TabIndicatorProps={{ style: { background: '#b6a1e4' } }}
                                    textColor="primary"
                                    sx={{
                                        backgroundColor: 'fundoCard', borderRadius: '15px',
                                    }}>
                                    <Tab label="Recebidas" />
                            ...
                            <Grid overflow={'auto'} maxHeight={'70vh'} className="gridTimes" //className={tipoAv === 0 ? "gridTimes" : "ocultar"} 
                                container spacing={4}>
                                <Container
                                    sx={{
                                        minWidth: 275,
                        ...
                        <Container maxWidth="md" className="divBotaoFim" disableGutters sx={{ display: 'flex' }}>
