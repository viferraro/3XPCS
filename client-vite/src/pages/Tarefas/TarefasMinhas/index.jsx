...
import {
    ...
    Button, Link, CardContent, Backdrop, CircularProgress, FormControl, Select,
    MenuItem, InputLabel, Badge, Grow, Tooltip
} from "@mui/material";
...
const TarefasMinhas = () => {
        ...
        if (!(projeto.projetoTarefa === -1)) {
            ...
            if (parseInt(statusSelecionado) === -1) {
                ...
                nvListaTarefas = (listaTarefas.filter(
                    tarefa => parseInt(tarefa.projetoTarefa) === parseInt(projeto.projetoTarefa)
                ));
    ...
    const filtrarStatus = (evento, status) => {
        ...
        //Se for todos, reitere o filtro...
            ...
            //Se houver projeto selecionado, filtre por ele
                ...
                // ! Filtrando o projeto selecionado 
                nvListaTarefas = (listaTarefas.filter(
                    tarefa => parseInt(tarefa.projetoTarefa) === parseInt(projetoSelecionado)
                ));
            }
        }
        setListaTarefasFiltrada(nvListaTarefas)
    ...
    const getTarefas = (usuario) => {
        Axios.put("http://localhost:1003/api/tarefas-view/usuario", {
            email: usuario,
        }).then(
            response => {
    ...
    const detalharTarefa = (tarefa) => {
        ...
        redirect('/tarefas/detalhar'//, { state: { usuario: locationState.usuario, admin: locationState.admin, tarefa: tarefa } }
        )
    }
        ...
        <div className={"minhas-tarefas"}>
            ...
            <div className={jaCarregou ? "principalTimes" : "ocultar"}>
                <ThemeProvider theme={MuiEstilosPersonalizados}>
                    <Container className="container-minha-tarefa"
                        disableGutters
                        ...
                        <Container disableGutters className="divBotaoFim" sx={{ paddingBottom: '1%', margin: '0', display: 'flex', justifyContent: 'space-between' }}>
                            <Button className="botao-tarefas-sigo" variant="contained" color="success"
                        ...
                        <Container className="container-filtro-minhas-tarefas" sx={{ display: 'flex', marginTop:'1.5em  ' }} container spacing={4}>
                            <Typography
                            ...
                            <FormControl
                                sx={{ display: 'flex', flexDirection: 'column', margin: '0.5em 0' }}>
                                <Typography
                                    className="titulo-minhas-tarefas"
                                    gutterBottom
                                    variant="body1"
                                ...
                                <Select className="select-filtro-minhas-tarefas"
                                ...
                                >
                                    {selectProjetos.map((projeto) => {
                                        return <MenuItem value={projeto.projetoTarefa}
                            ...
                            <FormControl
                                ...
                                >
                                    {selectStatus.map((status) => {
                                        return <MenuItem value={status.status}
                        ...
                        <Container className='container-exibe-minhas-tarefas'
                            sx={{ overflow: 'auto', display: 'flex', marginBottom:'1.5em', flexWrap: 'wrap', alignItems: 'flex-start' }}
                            //overflow={'auto'} 
                            maxHeight={'70vh'}>
                                ...
                                return (
                                    ...
                                    <Box sx={{
                                        ...
                                        display: 'flex'
                                            ...
                                            <Tooltip title={ tarefa.status ===  1 ? "Em andamento" : 
                                                        tarefa.status === 2 ? "Em andamento" :
                                                        tarefa.status === 3 ? "Concluída" :  "Inativada"
                                                        }
                                            placement="top"
                                            arrow
                                            >
                                                <Badge badgeContent={
                                                    <div >
                                                        <FaCircle color={corStatus[parseInt(tarefa.status)]} fontSize="1.7em" />
                                                    </div>
                                                }
                                                ></Badge>
                                            </Tooltip>
                        ...
                        <Button variant="contained" color="info" startIcon={<FaHandPointLeft />} onClick={voltar} sx={{ marginTop: '9px' }}>
                            Voltar
                        </Button>
