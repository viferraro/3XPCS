...
import {
    ...
    Button, Link, CardContent, CardActions, List,
    ListItem, ListItemText, FormControl, Select,
    MenuItem, Backdrop, CircularProgress, Badge, Tooltip
} from "@mui/material";
...
const TarefasProjeto = () => {
    ...
    const getTarefas = (codProjeto) => {
        Axios.get("http://localhost:1003/api/projetos/" + codProjeto + "/tarefas").then(
            response => {
            ...
            <div className={jaCarregou ? "principal-minhas-tarefas" : "ocultar"}>
                <ThemeProvider theme={MuiEstilosPersonalizados}>
                    <Container className="container-minha-tarefa"
                        ...
                        maxWidth="md">
                        <Container className="container-filtro-minhas-tarefas" sx={{ display: 'flex' }} container spacing={4}>
                            <Typography
                            ...
                            {listaTarefasFiltrada.map((tarefa) => {
                                return (
                                    <Box sx={{
                                        ...
                                        <Tooltip
                                            title={tarefa.status === 1 ? "Em andamento" :
                        ...
                        <Container className="containerVolta" disableGutters sx={{ display: 'flex', justifyContent: 'space-between' }}>
                            <Button variant="contained" color="info" startIcon={<FaHandPointLeft />}
                                onClick={voltar}
                                sx={{ display: 'flex', marginTop: '3%' }}>
                                Voltar
                            </Button>
                            <Button className="botao-tarefas-sigo" variant="contained" color="success"
                                startIcon={<FaBookOpen />} sx={{ display: 'flex', marginTop: '3%' }}
