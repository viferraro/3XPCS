...
// Imports MUI e Joy
import {
    Grid, Container, Typography, Card, Badge,
    Button, Link, CardContent, TextField, Chip, Avatar,
    Backdrop, CircularProgress, Tooltip,
    Snackbar, Alert, AlertTitle
} from "@mui/material";
import { ThemeProvider } from "@mui/material/styles";
import { FaRegCheckCircle, FaRegTimesCircle, FaPlusCircle, FaCircle, FaHandPointLeft } from 'react-icons/fa';
...
function TarefasDetalhar() {
    ...
    const [locationState, setLocationState] = React.useState({ admin: '', usuario: '', tarefa: '' });
    ...
    const getComentarios = (codTarefa) => {
        Axios.put("http://localhost:1003/api/tarefas/" + codTarefa + "/comentarios", {
            codTarefa: codTarefa,
        }).then(
            response => {
    ...
    const vrfTarefaESeguida = (tarefa, usuario) => {
        Axios.put("http://localhost:1003/api/tarefas/" + tarefa + "/seguida", {
            email: usuario,
        }).then(
            response => {
    ...
    const deixarDeSeguirTarefa = () => {
        Axios.put("http://localhost:1003/api/usuarios/" + locationState.usuario.email + "/tarefa/parar-seguir", {
    ...
    const seguirTarefa = () => {
        ...
        Axios.put("http://localhost:1003/api/usuarios/" + locationState.usuario.email + "/tarefa/seguir", {
            codTarefa: locationState.tarefa.codTarefa,
        }).then(
            response => {
                if (response.status === 200) {
    ...
    const detalharUsuario = (emailUsuario) => {
        ...
        if (emailUsuario !== null) {
            ...
            redirect("/perfil?" + emailUsuario//, { state: { usuario: locationState.usuario, admin: locationState.admin, usuarioPerfil: emailUsuario } }
            )
        }
    }
    ...
    const inserirComentario = () => {
        ...
        Axios.put("http://localhost:1003/api/tarefas/" + locationState.tarefa.codTarefa + "/addcomentario", {
            descricao: document.getElementById("txt_comentario").value,
            email: locationState.tarefa.usuarioResp,
        }).then(
            response => {
                if (response.status === 201) {
                    Axios.put("http://localhost:1003/api/tarefas/" + locationState.tarefa.codTarefa + "/seguidores",
                        {
                            email: locationState.tarefa.usuarioResp,
                            acao: "ADD",
                        ...
                        .then(resposta => {
                            if (resposta.status === 200) {
                                setMsgAlerts("Os usuários seguidores da tarefa e usuário receberão e-mails sobre o comentário!");
                                setAbreRemove(true)
                            }
                        })
    ...
    const concluiTarefa = () => {
        let msg = "";
        Axios.delete("http://localhost:1003/api/tarefas/" + locationState.tarefa.codTarefa,)
            .then(response => {
                if (response.status === 200) {
    ...
    let getTarefa = (id, usuario, admin) => {
        Axios.get("http://localhost:1003/api/tarefas/" + id).then(
            response => {
                if (response.status === 200) {
                    console.log(response.data)
                    setLocationState({ admin: admin, usuario: usuario, tarefa: response.data })
                }
            });
    }
    ...
    if (locationState.tarefa === '') {
        return <div className="detalhar-tarefa">
            <BarraLogoSemAuth />
    ...
    const detalharProjeto = (codProjeto) => {
        //Redirecionar o usuario para a página de seguir o outro usuario
        if (codProjeto !== null) {
            Axios.get("http://localhost:1003/api/projetos/" + codProjeto).then(
                response => {
                    if (response.status === 200) {
                        //Setando projeto no localStorage
                        localStorage.setItem("projeto", JSON.stringify(response.data))

                        redirect("/projetos/detalhar"//, { state: { usuario: locationState.usuario, admin: locationState.admin, projeto: response.data } }
                        )
                    }
                });
        }
            ...
            <div className="principal-detalha-tarefa">
                <ThemeProvider theme={MuiEstilosPersonalizados}>
                    ...
                    <Container disableGutters sx={{ paddingTop: '0' }} maxWidth="md">
                        ...
                        <Grid sx={{ maxHeight: '100%', overflow: 'auto', marginTop: '1.6em' }} item key={"Tarefa_" + locationState.tarefa.codTarefa} xs={12} sm={6} md={4} >

                            <Card className="card-detalha-tarefa"
                                ...
                                <CardContent
                                    ...
                                    >
                                        ...
                                        <Tooltip
                                            placement="top"
                                            arrow
                                            title={locationState.tarefa.status === 1 ? "Em andamento" :
                                                locationState.tarefa.status === 2 ? "Em andamento" :
                                                    locationState.tarefa.status === 3 ? "Concluída" : "Inativada"}>
                                            <Badge
                                                sx={{
                                                    marginLeft: '0.8em'
                                                }}
                                                badgeContent={
                                                    <div >
                                                        <FaCircle color={corStatus[parseInt(locationState.tarefa.status)]} fontSize="1.7em" />
                                                    </div>
                                                }
                                            >
                                    ...
                                    <Container className={(eTarefaDoUsuario && locationState.tarefa.status !== 3 && locationState.tarefa.status !== 4) ? "container-comentario-tarefa" : "ocultar"}
                                        height="10px"
                                        sx={{ display: 'flex' }}
                                    >
                                    ...
                                    <Container
                                        ...
                                        sx={{
                                            ...
                                            margin: '0.3em', width: 'fit-content', borderRadius: '5px'
                                        }}>
                                        {listaComentarios.map((comentario) => {

                                            return <Chip
                                                avatar={<Avatar src={comentario.urlImagem} />}
                                                label={comentario.nome + " - " + comentario.dtCriacao
                                    ...
                                    <Container disableGutters className="divBotaoFim"
                                        sx={{ display: 'flex', justifyContent: 'flex-end', margin: '0' }}
                                    >

                                        <Button className={
                                            eTarefaDoUsuario &&
                                                locationState.tarefa.status === 1 ? "botaoConcluir" : "ocultar"
                            ...
                            >
                                ...
                                <Button className={
                                    eTarefaDoUsuario ? "ocultar" :
                                        (tarefaESeguida ? "botaoAlterar" : "ocultar")
                                ...
                                <Button className={
                                    eTarefaDoUsuario ? "ocultar" :
                                        (tarefaESeguida ? "ocultar" : "botaoAlterar")
