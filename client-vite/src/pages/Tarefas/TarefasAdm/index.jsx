...
import { FaPlusCircle, FaBook, FaBookOpen, FaHandPointLeft, FaPencilAlt } from 'react-icons/fa';
...
const TarefasAdm = () => {
    ...
    const [locationState, setLocationState] = React.useState({ admin: '', usuario: '' });
    ...
    const getTarefas = () => {
        Axios.get("http://localhost:1003/api/tarefas").then(
            response => {
                ...
                response.data.forEach(elemento => {
                    let { nome, descricao, nomeProjeto, nomeIteracao, nomeUsuarioResp, status } = elemento;
    ...
    const columns = [
        { field: 'nome', headerName: 'Nome', headerClassName: 'headerTBL', flex: 2 },
        { field: 'descricao', headerName: 'Descrição', headerClassName: 'headerTBL', flex: 2 },
        { field: 'nomeProjeto', headerName: 'Projeto', headerClassName: 'headerTBL', flex: 2 },
        { field: 'nomeIteracao', headerName: 'Iteração', headerClassName: 'headerTBL', flex: 2 },
        { field: 'nomeUsuarioResp', headerName: 'Responsável', headerClassName: 'headerTBL', flex: 2 },
        { field: 'status', headerName: 'Status', headerClassName: 'headerTBL', flex: 2 }
    ];
    ...
    const criarTarefa = () => {
        redirect('/tarefas/criar'//, { state: { usuario: locationState.usuario, admin: locationState.admin } }
        )
    }

    // ! Função que vai redirecionar o usuário para ver as suas tarefas
    const minhasTarefas = () => {
        redirect('/tarefas'//, { state: { usuario: locationState.usuario, admin: locationState.admin } }
        )
    }
    ...
    const detalharTarefa = () => {
        ...
        redirect('/tarefas/detalhar'//, { state: { usuario: locationState.usuario, admin: locationState.admin, tarefa: tarefaSelecionada } }
        )
    }
    ...
    const alterarTarefa = () => {
        ...
        redirect('/tarefas/alterar'//, { state: { usuario: locationState.usuario, admin: locationState.admin, tarefa: tarefaSelecionada } }
        )
    }

    // ! Função que vai redirecionar o usuário para as tarefas seguidas
    const tarefasSeguidas = () => {
        redirect('/tarefas/seguidas'//, { state: { usuario: locationState.usuario, admin: locationState.admin } }
        )
    }
    ...
    return (
        <div className={"tarefas-adm"}>
            ...
            <Container className={jaCarregou ? "principal-tarefas-adm" : "ocultar"}
                sx={{ display: 'flex' }}
            >
                <ThemeProvider theme={MuiEstilosPersonalizados}>
                    <Container disableGutters className="divBotaoFim" sx={{ paddingBottom: '1%', margin: '0', display: 'flex', justifyContent: 'space-between' }}>
                        <Button variant="contained" color="success" startIcon={<FaPlusCircle />} onClick={criarTarefa} sx={{ display: 'flex' }}>
                            ADICIONAR TAREFA
                        </Button>
                        <Button variant="contained" color="warning" startIcon={<FaBook />} onClick={minhasTarefas} sx={{ display: 'flex' }}>
                            MINHAS TAREFAS
                        </Button>
                    </Container>

                    <div className="gridTarefas">
                        <DataGrid className={"tbl"}
                            autoHeight
                            ...
                            onSelectionModelChange={(ids) => {
                                ...
                                if (isNaN(idSelecionado))
                                    return setTemTarefaSelecionada(false);
                    ...
                    <Container disableGutters className="divBotaoFim" sx={{ display: 'flex', margin: '0' }}>                        
                        <Button variant="contained" color="info" startIcon={<FaHandPointLeft />} onClick={voltar}>
                            Voltar
                        </Button>
                        
                        <Button className="botaoAlterar" variant="contained" color="success"
                            startIcon={<FaBookOpen />} sx={{ display: 'flex' }}
                            onClick={tarefasSeguidas}
                        >
                            TAREFAS QUE SIGO
                        </Button>

                        <Button className={temTarefaSelecionada ? "botaoAlterar" : "ocultar"} variant="contained" color="info"
                            startIcon={<FaBook />} sx={{ display: 'flex' }}
                            onClick={detalharTarefa}
                        >
                            DETALHAR
                        </Button>

                        <Button className={temTarefaSelecionada ? "botaoAlterar" : "ocultar"} variant="contained" color="warning"
                            startIcon={<FaPencilAlt />} sx={{ display: 'flex' }}
            ...
            <Container
                className={jaCarregou ? "ocultar" : "principal-tarefas-adm"}>
                <Backdrop
