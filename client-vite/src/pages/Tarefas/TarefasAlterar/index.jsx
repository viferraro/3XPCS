...
// Imports MUI e Joy
import {
    Container, Typography, Box, Button, TextField, CssBaseline,
    Select, MenuItem, Snackbar, Alert, AlertTitle, Backdrop, CircularProgress 
} from "@mui/material";
...
const TarefasAlterar = () => {
    ...
    const [codIteracaoSelecionada, setCodIteracaoSelecionada] = React.useState('');
    ...
    const getUsuarios = (codProjeto) => {
        Axios.get("http://localhost:1003/api/projetos/" + codProjeto + "/usuarios").then(
            response => {
                console.log(response.data)
    ...
    const [emailUsuarioSelecionado, setEmailUsuarioSelecionado] = React.useState('');
    ...
    const getProjetos = () => {
        Axios.get("http://localhost:1003/api/projetos").then(
            response => {
    ...
    const atualizaTarefa = () => {
        ...
        if (vrfUsuarioProjeto === true) {
            Axios.put("http://localhost:1003/api/tarefas/" + locationState.tarefa.codTarefa, {
                ...
                .then((respostaCriaTarefa) => {
                    if (respostaCriaTarefa.status === 200) {
                        ...
                        setSair(true);
                        setMensagem(document.getElementById("nome-tarefa").value + " foi atualizada!");
                    }
                });
        }
        else {
    ...
    const fecharDialog = () => {
        setSair(false);
        redirect("/tarefas/adm"//, { state: { admin: true, usuario: locationState.usuario } }
        );
    }
                    ...
                    <ThemeProvider theme={MuiEstilosPersonalizados}>
                        ...
                        <Container sx={{ height: '100%' }} maxWidth="md">
                            <Box className="boxNovoTime"
                                sx={{ marginTop: '0' }}>
                                <Container className="container-nome-tarefa" sx={{ display: 'flex' }}>
                                    <Typography variant="body" component="h5" gutterBottom>
                                        Nome:
                                    </Typography>
                                    <TextField id="nome-tarefa"
                                        required
                                        variant="outlined"
                                ...
                                <Container sx={{ display: 'flex' }} className="container-projetos-tarefas containerDados">
                                    <Typography variant="body" component="h5" gutterBottom>
                                        Projetos:
                                    </Typography>
                                    <Select
                                        ...
                                        {radioButtons.map(value => (
                                            <label key={value}>
                                                <input
                                                    type="radio"
                                                    name="radio-group"
                                                    {...(value === statusSlc ? { checked: true } : { checked: false })}
