...
apikey.apiKey = "00C03F12D95E0045D0A39957B4EAC6B49850D9DC3269277ED5D879057B27B517C83F483D73A6526749AC9E64BAD23C14"
...
module.exports = {
    ...
    async alteraStatus(email, status) {
        try {
            ...
            return { message: "Status do usuário alterado com sucesso!", status: 201 }
        }
        catch (err) {
    ...
    // * Atualiza um usuário
    async updateUser(email, nome, funcao, github, linkedin, cep, numEnd, complEnd) {
        try {
            let pool = await poolPromise
            let data = await pool.request()
                .input("em", sql.VarChar, email)
                ...
                .query("UPDATE dbo.USUARIO SET nome=@nm, funcao=@fn, github=@gh, linkedin=@lk, CEP=@cp, CEP_numEnd=@cn, CEP_complemento=@cc WHERE email=@em");
    ...
    async userLogin(email, nome, urlImagem) {
        try {
            ...
            let data = await pool.request()
                ...
                .query("UPDATE dbo.USUARIO SET urlImagem=@im, nome=@nm WHERE email=@em");
    ...
    async addUser(emailNv, nome, status) {
        ...
        try {
            ...
            let data = await pool.request()
                ...
                .query("INSERT INTO dbo.USUARIO (email, nome, status) VALUES (@em, @nm, @st)");
                ...
                Content: {
                    Body: [
                        ElasticEmail.BodyPart.constructFromObject({
                            ContentType: "HTML",
                            Content: "Prezado <b>" + nome + "</b>, você recebeu permissões para acessar o sistema 3X. <br> "
    ...
    async updateUserAdmin(email, emailNovo, funcao, status) {
        try {
            ...
            let data = await pool.request()
                ...
                .input("fn", sql.VarChar, funcao)
                .input("st", sql.Numeric, status)
                .query("UPDATE dbo.USUARIO SET email=@emNv, funcao=@fn, status=@st WHERE email=@em");
    ...
    async addUserTeam(email, time) {
        try {
            ...
            let data = await pool.request()
                ...
                .query("INSERT INTO dbo.USUARIO_TIME (USUARIO_email, TIME_codTime) VALUES (@em, @tm)");
    ...
    async isUsuarioSeguidoPorUsuario(emailSeguido, emailUsuario) {
        ...
        try {
            ...
            let data = await pool.request()
                ...
                .query("SELECT * FROM dbo.USUARIO_SEGUE_USUARIO WHERE usuarioSeguidor=@em AND usuarioSeguido=@em2");
        ...
        try {
            ...
            if (parseInt(admin) === 1) {
                ...
                for (let i in data.recordset) {
                    ...
                    dashboard.qtdTimesUsuario = data.recordset[i].qtdTimesUsuario ? data.recordset[i].qtdTimesUsuario : dashboard.qtdTimesUsuario;
                    dashboard.qtdTimesSys = data.recordset[i].qtdTimesSys ? data.recordset[i].qtdTimesSys : dashboard.qtdTimesSys;
                    dashboard.qtdProjetosUsuario = data.recordset[i].qtdProjetosUsuario ? data.recordset[i].qtdProjetosUsuario : dashboard.qtdProjetosUsuario;
                    dashboard.qtdProjetosSys = data.recordset[i].qtdProjetosSys ? data.recordset[i].qtdProjetosSys : dashboard.qtdProjetosSys;
                    dashboard.qtdTarefas = data.recordset[i].qtdTarefas ? data.recordset[i].qtdTarefas : dashboard.qtdTarefas;
                    dashboard.qtdTarefasSys = data.recordset[i].qtdTarefasSys ? data.recordset[i].qtdTarefasSys : dashboard.qtdTarefasSys;
                    dashboard.qtdTarefasSemana = data.recordset[i].qtdTarefasSemana ? data.recordset[i].qtdTarefasSemana : dashboard.qtdTarefasSemana;
                    dashboard.qtdUsuarios = data.recordset[i].qtdUsuarios ? data.recordset[i].qtdUsuarios : dashboard.qtdUsuarios;
                }
                /*
                    qtdTimesUsuario
                    qtdTimesSys
    ...
    async avaliarUsuario(email, emailAvaliado, avaliacao, comentario) {
        try {
            ...
            let data = await pool.request()
                ...
                .query("INSERT INTO dbo.AVALIACAO_USUARIO VALUES (@em, @emA, @av, @cm)");
    ...
    async alterEntretenimento(email, spotify, movie_id, movie_type) {
        try {
            ...
            let data = await pool.request()
                ...
                .input("mid", sql.VarChar, movie_id.toString())
                .input("mty", sql.VarChar, movie_type)
                .query("UPDATE dbo.ENTRETENIMENTO SET spotify_uri=@sp, moviedb_id=@mid, moviedb_tipo=@mty WHERE USUARIO = @em");
