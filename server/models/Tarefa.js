...
module.exports = {
    ...
    async getTarefasViewByUsuario(emailUsuario) {
        try {
            ...
            let data = await pool.request()
                ...
                .query("SELECT * FROM TAREFAS_ITERACAO_PROJETOS_USUARIOS WHERE usuarioResp LIKE @em AND (status = 1 OR status = 3)")
    ...
    // * Atualiza uma tarefa
    async updateTarefa(codTarefa, nome, descricao, status, codIteracaoFK, usuarioResp) {
        try {
            let pool = await poolPromise
            let data = await pool.request()
                .input("id", sql.Numeric, codTarefa)
                ...
                .query("UPDATE dbo.TAREFA SET nome=@nm, descricao=@de, status=@st, codIteracaoFK=@it, usuarioResp=@us WHERE codTarefa=@id");
    ...
    async addTarefa(nome, descricao, status, codIteracaoFK, usuarioResp) {
        try {
            ...
            let data = await pool.request()
                ...
                .input("it", sql.Numeric, codIteracaoFK)
                .input("us", sql.VarChar, usuarioResp)
                .query("INSERT INTO dbo.TAREFA(nome, descricao, status, codIteracaoFK, usuarioResp) VALUES (@nm, @de, @st, @it, @us)");
    ...
    async addComentario(codTarefa, descricao, emailUsuario) {
        try {
            ...
            let data = await pool.request()
                ...
                .input("de", sql.VarChar, descricao)
                .input("em", sql.VarChar, emailUsuario)
                .query("INSERT INTO dbo.COMENTARIOS (codTarefa, descricao, usuario) VALUES (@ct, @de, @em)");
    ...
    async getListaUsuariosGatilho(codTarefa, emailUsuario, acao, nomeTarefa, nomeRespTarefa, comentario) {
        ...
        try {
            ...
            for (let i = 0; i < data.rowsAffected; i++) {
                usuarios.push(new ElasticEmail.EmailRecipient(data.recordset[i].usuarioSeguidor.toString()))
                    ...
                    Content: {
                        Body: [
                            ElasticEmail.BodyPart.constructFromObject({
                                ContentType: "HTML",
                                Content: "A tarefa <b>" + nomeTarefa + "</b> recebeu um novo comentário de "
                                    + nomeRespTarefa + ": <br>" + comentario
                            })
                        ],
                ...
                let email = ElasticEmail.EmailMessageData.constructFromObject({
                    ...
                    Content: {
                        Body: [
                            ElasticEmail.BodyPart.constructFromObject({
                                ContentType: "HTML",
                                Content: "A tarefa <b>" + nomeTarefa + "</b> foi concluida por "
                                    + nomeRespTarefa + "!"
                            })
                        ],
